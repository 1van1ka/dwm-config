...
static void
refresh(Display *dpy, Window win, int screen, struct tm time, cairo_t *cr, cairo_surface_t *sfc, int color)
{
	static char tm[24]="";
	int xpos, ypos;
	...
	cairo_move_to(cr, xpos, ypos + textsize / 2);
	cairo_set_source_rgb(cr,
		textcolors[INIT][0],
		textcolors[INIT][1],
		textcolors[INIT][2]);
	cairo_set_font_size(cr, textsize / 2.5);
...
{ /*Thread that keeps track of time and refreshes it every 2 seconds */
	...
	while (1){
		...
		for (int k=0;k<displayData->nscreens;k++)
			refresh(displayData->dpy, displayData->locks[k]->win,
				...
				displayData->crs[k], displayData->surfaces[k], INIT);
...
readpw(Display *dpy, struct xrandr *rr, struct lock **locks, int nscreens,
	...
	int num, screen, running = 1, failure;
	unsigned int len = 0, color = 0;
	KeySym ksym;
	XEvent ev;
	...
	while (running && !XNextEvent(dpy, &ev)) {
		...
		if (ev.type == KeyPress) {
			...
			color = (len && !failure) ? INPUT : (failure ? FAILED : INIT);

			if (running) {
				pthread_mutex_lock(&mutex);
				for (screen = 0; screen < nscreens; screen++) {
                    if (locks[screen]->bgmap) XSetWindowBackgroundPixmap(dpy, locks[screen]->win, locks[screen]->bgmap);
                    else XSetWindowBackground(dpy, locks[screen]->win, locks[screen]->colors[0]);
					XClearWindow(dpy, locks[screen]->win);
					time_t rawtime;
                    time(&rawtime);
	                refresh(dpy, locks[screen]->win, locks[screen]->screen, *localtime(&rawtime), crs[screen], surfaces[screen], color);
				}
				pthread_mutex_unlock(&mutex);
			}
...
main(int argc, char **argv) {
	...
	XSetInputFocus(dpy, locks[0]->win, RevertToParent, CurrentTime);
	XSync(dpy, 0);
	...
	cairo_surface_t **surfaces;
	cairo_t **crs;
    if (!(surfaces=calloc(nscreens, sizeof(cairo_surface_t*))) || !(crs=calloc(nscreens, sizeof(cairo_t*)))) {
		die("Out of memory");
	}
